...
`;
    ...
    for (let i = 1; i < limiter + 1; i++) {
      ...
      if (i == 4) {
        ...
        content += `<p><span class="navi text-left"><a class="badge badge-warning" href="/${dataKw[1]
          .replace(/\s/g, "-")
          .toLowerCase()}/">${ucwords(dataKw[1])}</a></span>
        <span class="navi text-left"><a class="badge badge-dark" href="/${dataKw[2]
          .replace(/\s/g, "-")
          .toLowerCase()}/">${ucwords(dataKw[2])}</a></span>
        <span class="navi text-left"><a class="badge badge-success" href="/${dataKw[3]
          .replace(/\s/g, "-")
          .toLowerCase()}/">${ucwords(dataKw[3])}</a></span>
        <span class="navi text-left"><a class="badge badge-danger" href="/${dataKw[4]
          .replace(/\s/g, "-")
          .toLowerCase()}/">${ucwords(dataKw[4])}</a></span></p>`;
      }
      content += `<p><img id="img" width="100%" src="/img/placeholder.svg" data-src="${img[i]["image"]}" alt="${img[i]["title"]}" title="${img[i]["title"]}" onerror="this.onerror=null;this.src='${img[i]["thumbnail"]}';" class="center" loading="lazy" />${text[i]}</p>`;
    }
